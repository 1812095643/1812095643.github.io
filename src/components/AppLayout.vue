...
function resetPageState() {
  // 清除之前的事件监听器（除了基础的）
  const scrollListeners = cleanupFns.filter(fn => fn.toString().includes('scroll'))
...
onMounted(async () => {
  ...
  try {
    await loadScriptsInOrder(['/js/anime.js', '/js/firework.js', '/js/main.min.js'])
  } catch (error) {
  ...
  const onMouseMove = (e: MouseEvent) => {
    document.querySelectorAll<HTMLElement>('.magical').forEach((el) => {
      ...
      if (show) {
        show.style.opacity = x >= 0 && x <= el.clientWidth && y >= 0 && y <= el.clientHeight ? '1' : '0'
      }
    })
  }
    ...
    animationFrameId = requestAnimationFrame(() => {
      document.querySelectorAll<HTMLElement>('.field-zone').forEach((field) => {
        ...
        if (magneticCore) {
          ...
          magneticCore.style.transition = 'transform 0.08s cubic-bezier(0.23, 1, 0.32, 1)'
        ...
        particles.forEach((particle, index) => {
          ...
          // 大幅增加粒子移动强度和范围
          const attractionStrength = Math.max(0, 1 - particleDistance / 350) * (0.4 + index * 0.12)
          const maxParticleMove = 25 // 增加最大移动距离
          const particleMove = Math.min(attractionStrength * 35, maxParticleMove)
          
          const particleX = Math.cos(particleAngle) * particleMove
          const particleY = Math.sin(particleAngle) * particleMove
          
          particle.style.transition = 'transform 0.1s cubic-bezier(0.23, 1, 0.32, 1), box-shadow 0.08s ease'
          particle.style.transform = `translate(${particleX}px, ${particleY}px) scale(${1 + attractionStrength * 0.6})`
        ...
        if (centralOrb) {
          ...
          centralOrb.style.transition = 'transform 0.1s cubic-bezier(0.23, 1, 0.32, 1), box-shadow 0.08s ease'
        ...
        fieldLines.forEach((line) => {
          ...
          line.style.opacity = Math.min(0.4 + lineIntensity * 0.5, 0.9).toString() // 增加透明度变化
          line.style.boxShadow = `0 0 ${4 + lineIntensity * 12}px rgba(99, 102, 241, ${lineIntensity * 0.5})` // 增加发光变化
        })
      })
  ...
  cleanupFns.push(() => document.removeEventListener('mousemove', onMouseMovePupil))
  ...
  const closeBtn = document.querySelector('.menu-close-btn') as HTMLElement | null
  ...
  const onResize = () => { if (window.innerWidth > 968) closePanel() }
  ...
  const bookObserver = new IntersectionObserver((entries, obs) => {
    entries.forEach((entry) => {
      ...
      if (url && front) {
        const img = new Image()
        img.src = url
        img.onload = () => { front.style.backgroundImage = `url('${url}')` }
      }
      obs.unobserve(el)
    })
  }, { threshold: 0.1 })
  document.querySelectorAll<HTMLElement>('.book-item').forEach((el) => bookObserver.observe(el))
  ...
  const showCopied = () => {
    ...
    emailTimer = window.setTimeout(() => { copied.style.opacity = '0' }, 1000)
  }
